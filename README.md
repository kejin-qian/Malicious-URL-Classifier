...
Here are some instructions on how to run the python scripts in terminal: 

0. To clone my repo to your local, run

`git clone https://github.com/kejin-qian/Malicious-URL-Classifier.git`

Instructions on running scripts in terminal:

- Create a new environment by `conda create -n envName python=3.6` and activate your new environment by `conda activate envName`

- "cd" to the project  folder

- Switch to the right branch for this mid-project check

	`git checkout midproject`

- run `pip install -r requirements.txt` to install all the packages needed for files submitted as mid project check.
...
2. download_file.py
	...
	`python src/download_file.py --path_of_object_to_download=<path to the file on S3> --bucket_name=<S3 bucket name> --output_file_path=<path to save the downloaded file>`
	
3. upload_file.py

	`python src/upload_file.py --input_file_path=<path to the file to be uploaded> --bucket_name=<S3 bucket name> --output_file_path=<path to where you want to store the file on S3>`
	
4. create_database.py 
	To create database schema on RDS: 
	
	`python src/sql/create_database.py --RDS=True`
	
	To create database schema locally in sqlite:
	
	`python src/sql/create_database.py --RDS=False`
